...
def predict(path_to_normalized_data, path_to_encoder, test_size, size, min_files, path_to_result, path_to_classifier):
    ...
    results = train_sklearn_xgb_classifier(x_train, y_train, x_test, y_test, path_to_classifier=path_to_classifier, full=True)
    if path_to_result:
